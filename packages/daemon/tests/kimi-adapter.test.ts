 ...
 * docs/research/kimi-code-cli.md shapes and ADR-0025 flags-only spawn
 * (no live binary in this environment).
 */
...
describe("kimi adapter — registration", () => {
  it("is registered as a built-in with childChannel mcp", () => {
    ...
    // Adapter defaultModel/defaultEffort removed in #284 — allowlist default
    // (ADR-0014) is the only spawn-default mechanism.
    expect("defaultModel" in (adapter as object)).toBe(false);
    expect("defaultEffort" in (adapter as object)).toBe(false);
  });
...
describe("kimi adapter — prepare argv (golden)", () => {
  ...
  it("passes namespaced model with -m when set, omits it otherwise (ADR-0025)", async () => {
    const adapter = createKimiAdapter({});
    // Live-verified (#290): bare form rejected; namespaced form accepted.
    const namespaced = "kimi-code/kimi-for-coding";
    const withModel = await adapter.prepare(spec({ model: namespaced }), HUB);
    expect(withModel.argv).toContain("-m");
    expect(withModel.argv).toContain(namespaced);
    // Model is in the flags region after -p <prompt>.
    expect(withModel.argv.indexOf("-m")).toBeGreaterThan(withModel.argv.indexOf("-p"));
    const mIdx = withModel.argv.indexOf("-m");
    expect(withModel.argv[mIdx + 1]).toBe(namespaced);
  ...
  it("maps model onto KIMI_MODEL_NAME env family as well as -m (task wins)", async () => {
    const plan = await createKimiAdapter({}).prepare(
      spec({ model: "kimi-code/kimi-k2.5" }),
      HUB,
    );
    expect(plan.env.KIMI_MODEL_NAME).toBe("kimi-code/kimi-k2.5");
    expect(plan.argv).toContain("kimi-code/kimi-k2.5");
  });
...
describe("kimi adapter — flags-only spawn (ADR-0025, no home isolation)", () => {
  it("does not set KIMI_CODE_HOME (child uses operator real home)", async () => {
    const plan = await createKimiAdapter({}).prepare(spec(), HUB);
    expect("KIMI_CODE_HOME" in plan.env).toBe(false);
  });

  it("does not materialize a generated config.toml", async () => {
    const plan = await createKimiAdapter({}).prepare(
      spec({ model: "kimi-code/kimi-for-coding" }),
      HUB,
    );
    expect(plan.files.every((f) => !f.path.endsWith("config.toml"))).toBe(true);
    expect(plan.files.map((f) => f.path)).toEqual([KIMI_MCP_JSON_REL]);
  });
  ...
  it("TaskSpec model/effort override ambient KIMI_MODEL_* name/effort", async () => {
    const plan = await createKimiAdapter({
      ...
      spec({ model: "kimi-code/delegated-model", effort: "high" }),
      HUB,
    );
    expect(plan.env.KIMI_MODEL_NAME).toBe("kimi-code/delegated-model");
    expect(plan.env.KIMI_MODEL_THINKING_EFFORT).toBe("high");
    expect(plan.argv).toContain("-m");
    expect(plan.argv).toContain("kimi-code/delegated-model");
  });
});
...
describe("kimi adapter — no-resume fallback (fresh semantics)", () => {
  ...
  it("resume re-materializes MCP only (no home/config) and keeps env posture", async () => {
    const adapter = createKimiAdapter({});
    const s = spec({ sessionId: "sess-1", sandbox: "read-only" as SandboxMode });
    const resumed = await adapter.resume(s, HUB);
    expect("KIMI_CODE_HOME" in resumed.env).toBe(false);
    expect(resumed.files.map((f) => f.path)).toEqual([KIMI_MCP_JSON_REL]);
