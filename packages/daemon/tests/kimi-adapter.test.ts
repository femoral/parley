...
describe("kimi adapter — isolated home & env (no provider-key passthrough)", () => {
  ...
  it("materializes generated config.toml under the isolated home", async () => {
    ...
    // Spawn-config default restored (finding 7) — deterministic headless spawn
    // when TaskSpec.model is null. Separate from the allowlist default combo.
    expect(config!.contents).toContain('default_model = "kimi-for-coding"');
  });

  it("pins default_model in isolated config when TaskSpec names a model", async () => {
    const plan = await createKimiAdapter({}).prepare(
