...
describe("migration (#233)", () => {
  it("creates runs + deliverables and adds run address columns on tasks", () => {
    ...
    // durability + placement, #317 git-auth, #329 capabilities_updated_at —
    // so the pre-runs snapshot is v24. Pinned absolutely — a relative offset
    // silently retargets this boundary on every appended migration (#389).
    // appending further entries.
    const prev = openDatabaseUpTo(homePaths(home), 24);
