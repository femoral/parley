...
describe("projectInspector projects the Q&A tab (#68)", () => {
  it("is empty when there's no history and no outstanding question", () => {
    const view = projectInspector(detail({ question: null }), NO_LOGS, []);
    expect(view.qa).toEqual([]);
  });

  it("appends the live outstanding question with a null answer", () => {
    const view = projectInspector(detail({ question: "Which shoal?" }), NO_LOGS, []);
    expect(view.qa).toEqual([{ question: "Which shoal?", answer: null }]);
  });

  it("carries remembered answered turns and appends a new outstanding question after them", () => {
    const view = projectInspector(
      detail({ question: "Deep or shallow anchorage?" }),
      NO_LOGS,
  ...
  it("shows only remembered history once the question resolves (no outstanding entry)", () => {
    const view = projectInspector(detail({ question: null }), NO_LOGS, [
      { question: "Which shoal?", answer: "The northern one." },
    ]);
    expect(view.qa).toEqual([{ question: "Which shoal?", answer: "The northern one." }]);
  });

  it("doesn't re-append a just-answered question the polled detail still carries", () => {
    // Right after an answer, `useTaskDetail`'s polled envelope can still hold
    // the answered question for one interval — the transcript must not show
    // the same turn answered and outstanding at once.
    const view = projectInspector(detail({ question: "Which shoal?" }), NO_LOGS, [
      { question: "Which shoal?", answer: "The northern one." },
    ]);
    expect(view.qa).toEqual([{ question: "Which shoal?", answer: "The northern one." }]);
