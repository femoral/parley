...
Contrast figures below are measured against **flat** `--parchment-bg` (`#efe0bd`) unless a second stop is named. The parchment the palette was ratified against is not flat — the #224 sheet is `linear-gradient(168deg, #f4e7c8, var(--parchment-bg) 46%, #e2cfa4)` plus stain layers and an inset edge wash. The gradient end alone is `#e2cfa4`, but the **true darkest composited paper pixel** (inset-shadow fringe, ink hidden) is **`#d1c1a5`** (L ≈ 0.544). AA math for text on the sheet must use that darker stop. See the **Textured-sheet AA rule** under Named Rules.

**Pen weights** (route, marginalia, not-yet-sailed marks — not status):
- **Chart Ink** (`--ink-chart` `#5b3a24`): the pen the whole map is drawn in. ≈7.74:1 on flat `#efe0bd` (≥4.5:1 AA); ≈6.60:1 on `#e2cfa4`; ≈5.72:1 on darkest composited `#d1c1a5`.
...
- **Live Vermilion** (`--ink-live` `#a8331f`): under way. Glyph: **✦**. ≈5.09:1 on flat `#efe0bd` (≥4.5:1 AA); ≈4.34:1 on `#e2cfa4` (fails AA — see textured-sheet rule). Use only on a light underlay no darker than flat parchment.
...
**The Textured-sheet AA Rule.** Chart-ink AA claims against flat `--parchment-bg` (`#efe0bd`) do not automatically hold on a textured or gradient sheet. The ratified #224 gradient ends at `#e2cfa4`, and the **composited sheet** (gradient + stains + inset edge wash) reaches a true darkest paper of **`#d1c1a5`** at the fringe. On `#e2cfa4`, `--ink-chart-soft` (≈4.21:1) and `--ink-live` (≈4.34:1) already fail normal-text AA while `--ink-chart` holds ≈6.60:1; on `#d1c1a5`, `--ink-chart` still holds ≈5.72:1. **Constraint:** text that must be read — including state glyphs and the "ahead" `?` — may use `--ink-chart-soft` or `--ink-live` only where the surface under that text is no darker than flat `#efe0bd`. Over any darker parchment region (including `#e2cfa4` stops, the `#d1c1a5` fringe, stains, or edge washes), step that text up to `--ink-chart`. `--ink-done` and `--ink-fail` still clear AA at `#e2cfa4`; `--ink-chart-ghost` never carries readable text.
...
### Navigation / Tabs
- **Style:** Cinzel 700 tracked caps; inactive tabs sit in faint ink, the active tab warms to brass with a brass underline/frame. Selection and hover use alpha tints of brass (`rgba(240,194,90,0.06–0.12)`), never a fill swap. Roster **selection is carried on a solid brass border plus an inset brass leading rail** (edge weight, not a third tint stop): the selected wash stays at the row's rest tint so text contrast never drops relative to unselected, while hover may still warm a selected row to `0.12` (#260).
...
### Pip track (roster)
The run roster's progress strip under each run option. Four kinds, each a short bar of luminous state ink on the wood row:
...
**Colour + height.** Kind is carried as **colour and height**, never colour alone. Non-fail pips sit at **3px**; a `fail` pip is the only kind that breaks the track's line, at the full track height of **5px**. The track box itself is fixed at 5px with `align-items: center`, so the row's height does not grow when a fail appears. Size/silhouette is the second cue required by the Do's ("icon, shape, label, or position").
...
**Aggregation tick.** When a run's bound exceeds the visible cap (`ROSTER_PIP_VISIBLE_CAP`, 20), the track shows at most 20 severity-aggregated segments and appends a quiet **6×5px** mark of three brass hairlines (`.pc-roster__pips-cap`). That tick means "the bound is larger than what is drawn"; it is not a kind, has no legend elsewhere in the UI, and is sighted-only — the full bound and kind counts live on the run option's `aria-describedby` summary. The track itself stays `aria-hidden`.
