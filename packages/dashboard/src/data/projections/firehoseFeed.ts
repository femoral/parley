 ...
 *
 * useSnapshot does not expose raw SSE events; the right-rail firehose diffs
 * tasks and runs and projects lines via {@link projectFirehoseLine}. Gate-VERB
 * lines are deferred (#360) — run.blocked surfaces as a hold, never approve/reject.
 */
import type { RunSummary, TaskEnvelope } from "@useparley/core";
import { projectFirehoseLine, workflowByRunId, type FirehoseInput } from "./firehose.js";
...
export function firehoseTone(line: FirehoseLine): string {
  ...
  if (
    s === "awaiting_answer" ||
    line.event.includes("question") ||
    line.event.includes("blocked")
  ) {
    return "awaiting";
  }
  ...
  if (
    s === "running" ||
    line.event.includes("started") ||
    line.event.includes("running")
  ) {
    return "running";
  }
