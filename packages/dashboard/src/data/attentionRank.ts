/**
 * Canonical attention ranking (DESIGN.md + PRODUCT.md first principle).
 *
 * Order: awaiting_answer → stalled → failed → running → queued → pending →
 * completed → cancelled. Then by age ascending (oldest first within a rank —
 * older asks surface sooner).
 *
 * Shared by the fleet tables, attention queue, and n/⇧N cycle order.
 * Do not re-implement this ranking elsewhere.
 */
...
/** Task states that need the orchestrator (header pill + attention queue). */
export const ATTENTION_TASK_STATES = new Set([
  "awaiting_answer",
  "stalled",
  "failed",
]);
...
export function isHeldGate(summary: {
  state: string;
  block?: { reason?: string } | null;
}): boolean {
  return summary.state === "blocked" && summary.block?.reason === "gate";
}
