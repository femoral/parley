 ...
 * + whole-run eval / run metrics (#243 / ADR-0020).
 *
...
/**
 * `parley run approve|reject|redirect|finish|status|get|eval|metrics …`
 */
export async function runRun(ctx: CliContext, args: string[]): Promise<number> {
  ...
  if (verbRaw === undefined) {
    throw new UsageError(
      "run: a verb is required (status | get | eval | metrics | approve | reject | redirect | finish)",
    );
  }
  ...
  // #243 — keep this branch small; sibling #242 adds fork/cancel nearby.
  ...
  if (!isGateVerb(verbRaw)) {
    throw new UsageError(
      `run: unknown verb "${verbRaw}" (expected status | get | eval | metrics | approve | reject | redirect | finish)`,
    );
  }
