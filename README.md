...
| `antigravity` | approximate (permissions.allow read tools only; host-wide reads, silent deny on write/command) | none (dangerously-skip-permissions; no write confinement (path-scoped allow rules do not work)) | enforced (dangerously-skip-permissions, no --sandbox) | refused (no network lever exists (research §5); prepare refuses rather than under-isolate) |
