...
| `@useparley/daemon` | Engine, adapters, sqlite storage (`db.ts`), HTTP server, MCP channel, worktree/lock/discovery/report — everything under `src/daemon/`. Owns the `better-sqlite3` dependency. | core |
...
- **Build tool: tsdown** (falling back to tsup if tsdown misbehaves) per package.
  ESM-only output + `.d.ts` to `dist/`; `better-sqlite3` external (it is a plain
  runtime dependency of daemon, never bundled).
...
- Each package: `"type": "module"`, `"engines": { "node": ">=22" }`,
  `exports` map pointing at `dist/`, `files` limited to `dist` (+ `bin`, `skills`
  for cli), `publishConfig: { "access": "public" }`.
- README (cli package) must note: pnpm ≥10 blocks install scripts by default —
  users must approve the `better-sqlite3` build
...
- Versioning & release pipeline (changesets vs fixed version, CI publish) —
  separate map ticket.
- `node:sqlite` migration — issue #54, after the split lands.
